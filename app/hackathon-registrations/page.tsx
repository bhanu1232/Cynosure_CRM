    ...
    const handleVerification = async (registration: Registration, isVerified: boolean) => {
        ...
        try {
            ...
            try {
                ...
                console.log(`Attempting to send ${isVerified ? 'verification' : 'rejection'} email to:`, recipientEmail);
                const emailData = {
                    to: recipientEmail,
                    name: recipientName,
            ...
            // Remove from separateRegistrations collection
            ...
            <div className="sm:flex sm:items-center mb-6">
                <div className="sm:flex-auto">
                    ...
                    <p className="mt-2 text-sm text-gray-700">
                        A list of all hackathon registrations and their verification status.
                    </p>
                </div>
            ...
            <div className="mb-6">
                <div className="max-w-xl">
                    <label htmlFor="search" className="block text-sm font-medium text-gray-700">
                        Search Hackathon Registrations
                    </label>
                                    ...
                                    {registrations.length === 0 ? (
                                        <tr>
                                            <td colSpan={9} className="py-8 text-center text-sm text-gray-500">
                                                No hackathon registrations found
                                            </td>
                                        </tr>
                                    ) : (
                                        registrations.map((registration) => (
                                            <tr key={registration.id}>
                                                <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">
                                                    ...
                                                    <div className="text-gray-500 text-xs">({registration.registrationType})</div>
                                                </td>
                                                <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                    <div className="text-gray-900">{registration.leaderName || registration.name}</div>
                                                    <div className="text-xs">{registration.teamName || 'Individual'}</div>
                                                </td>
                                                <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                    <div>{registration.leaderEmail || registration.email}</div>
                                                    <div className="text-xs">{registration.leaderPhone || registration.phone}</div>
                                                </td>
                                                <td className="px-3 py-4 text-sm text-gray-500 max-w-xs truncate" title={registration.participants?.join(', ')}>
                                                    {registration.participants && registration.participants.length > 0 ? registration.participants.join(', ') : 'N/A'}
                                                </td>
                                                <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                    {registration.collegeName || 'N/A'}
                                                </td>
                                                <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                    <div className="text-gray-900">₹{registration.fee}</div>
                                                    <div className="text-xs">{registration.paymentId}</div>
                                                </td>
